...
extension UIImage {
    func crop( rect: CGRect) -> UIImage {
        ...
        //rect.origin.x*=2.56
        //rect.origin.y*=2.56
        rect.size.width*=2.56
        rect.size.height*=2.56
        print("While Cropping",rect)
...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    var ImageFullHeight: CGFloat = 0
    var imageWidth: CGFloat = 0
    var imageHeight: CGFloat = 0
    var imageWidthratio: CGFloat = 0
    var imageScaleDownRatio: CGFloat = 0
    var imageHeightratio: CGFloat = 0
    ...
    func show(_ image: UIImage) {
        ...
        ImageFullWidth = fullImageWidth
        ImageFullHeight = fullImageHeight
        print("fULL IMAGE WIDTH",fullImageWidth)
        print("Full image height",fullImageHeight)
        let imageFrame = imageView.frame
        print("ImageFrame of image view",imageFrame)
        let widthRatio = fullImageWidth / imageFrame.width
        let heightRatio = fullImageHeight / imageFrame.height
        self.imageWidthratio = widthRatio
        self.imageHeightratio = heightRatio
        // ScaleAspectFit: The image will be scaled down according to the stricter dimension.
        let scaleDownRatio = max(widthRatio, heightRatio)
        self.imageScaleDownRatio = scaleDownRatio
        print(scaleDownRatio)
        // Cache image dimensions to reference when drawing CALayer paths.
        imageWidth = fullImageWidth / scaleDownRatio
        imageHeight = fullImageHeight / scaleDownRatio
        print("image width cached for reference when drawing CALayer paths",imageWidth)
        print(imageHeight)
    ...
    fileprivate func handleDetectedRectangles(request: VNRequest?, error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            var filtered_results = [VNRectangleObservation]()
            for (index, rect1) in results.enumerated() {
                let remainingRects = results[0...index]+results[index+1..<results.count]
                var rect_flag=0
                for rect2 in remainingRects {
                    if !rect1.boundingBox.equalTo(rect2.boundingBox){
                        if rect2.boundingBox.contains(rect1.boundingBox){
                            rect_flag=1
                        }
                    }
                }
                if rect_flag==0 {
                    filtered_results.append(rect1)
                }
            }
            self.draw(rectangles: filtered_results, onImageWithBounds: drawLayer.bounds)
    ...
    {
        let imageViewScale = max(ImageFullWidth / viewWidth,
                                 ImageFullHeight / viewHeight)
    ...
    fileprivate func draw(rectangles: [VNRectangleObservation], onImageWithBounds bounds: CGRect) {
        ...
        for observation in rectangles {
            ...
            let rectLayer = shapeLayer(color: .blue, frame: rectBox)            // Add to pathLayer on top of image.
            pathLayer?.addSublayer(rectLayer)
            let myImage = scaleAndOrient(image:self.imageView.image! )
            let croppedImage = cropImage(myImage, toRect: rectLayer.frame, viewWidth: self.imageWidth, viewHeight: self.imageHeight)
            //print("Layer Frame",rectLayer.frame)
            //let croppedImage=myImage.crop(rect:rectLayer.frame)
